...
(defun shift-line (del-count)
  (do ((y (- *field-height* 1))) ((or (< y 0) (<= del-count 0)))
    (let ((line-count 0))
      (dotimes (x *field-width*)
        (incf line-count (aref *field* x y)))
      (when (/= line-count 0)
      ...
      (:keydown (:keysym keysym)
                (if (sdl2:scancode= (sdl2:scancode-value keysym)
                    ...
                    (case (sdl2:scancode keysym)
                      (:scancode-left (move-piece +piece-left+))
                      (:scancode-right (move-piece +piece-right+))
                      (:scancode-down (move-piece +piece-down+))
                      (:scancode-z (turn-piece)))))
      (:idle ()
             (sdl2:set-render-draw-color renderer 0 0 0 255)
             (sdl2:render-clear renderer)

             (unless (move-piece +piece-down+)
               (piece-to-field)
